...
transform = transforms.Compose([
    transforms.Resize(331),  # Adjusted size for NasNet
...
# Model setup
model = torch.hub.load('pytorch/vision:v0.10.0', 'nasnetalarge', pretrained=True)  # Load NasNet model
model.classifier = nn.Sequential(
    nn.Dropout(dropout_rate),
    nn.Linear(model.classifier.in_features, len(dataset.classes))  # Adjust for number of classes
)
